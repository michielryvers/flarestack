...
if(!["plan","deploy","destroy"].includes(mode))throw new Error("Usage: bun scripts/deploy-cloud.ts [plan|deploy|destroy]");
const endpoint=process.env.FLARESTACK_DEPLOY_OTLP_ENDPOINT??"http://127.0.0.1:4320";
const dashboardUrl=process.env.FLARESTACK_DEPLOY_DASHBOARD_URL??"http://127.0.0.1:18889";
const grpcEndpoint=process.env.FLARESTACK_DEPLOY_OTLP_GRPC_ENDPOINT??"http://127.0.0.1:4321";
const logs=new LocalLogs(endpoint);
const dashboard=Bun.spawn(["aspire","dashboard","run","--non-interactive","--allow-anonymous","--frontend-url",dashboardUrl,"--otlp-http-url",endpoint,"--otlp-grpc-url",grpcEndpoint],{cwd:root,stdout:"ignore",stderr:"pipe"});
const dashboardErrors=readLines(dashboard.stderr,line=>console.error(line));
let child:Bun.Subprocess|undefined;
const stop=()=>{child?.kill("SIGTERM");};process.on("SIGINT",stop);process.on("SIGTERM",stop);
try {
  for(let i=0;i<60;i++){if(dashboard.exitCode!==null)throw new Error("Deployment log receiver exited; check its ports");try{if((await fetch(dashboardUrl)).ok)break;}catch{}if(i===59)throw new Error("Deployment log receiver unavailable");await Bun.sleep(500);}
  if(mode!=="destroy") {
  ...
  const args=["bun",cli,mode==="destroy"?"destroy":"deploy","--config","cloud.run.ts","--stage",config.stage,"--yes",...(mode==="plan"?["--dry-run"]:[])];
...
} finally {await logs.shutdown();dashboard.kill("SIGINT");await Promise.race([dashboardErrors,Bun.sleep(1000)]);process.off("SIGINT",stop);process.off("SIGTERM",stop);}
