...
export const Database = Cloudflare.D1.Database("Database", { migrations: `${import.meta.dirname}/../../../samples/Todo/migrations` });
export class Auth extends Cloudflare.Worker<Auth>()("Auth", {
  ...
  const auth = yield* BetterAuth(authOptions(publicOrigin));
  if (!globalThis.__ALCHEMY_RUNTIME__) {
    const { provisionClient } = yield* Effect.promise(() => import("../../../src/alchemy/provision-client.ts"));
    yield* provisionClient(publicOrigin);
  }
  return {
