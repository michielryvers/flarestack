...
{
    [Theory]
    [InlineData("/todos", true)] [InlineData("//evil.test", false)] [InlineData("/\\evil.test", false)] [InlineData("https://evil.test", false)] [InlineData("/todos\n", false)]
    public void ReturnUrlIsLocal(string url, bool valid) => Assert.Equal(valid, FlarestackAuthentication.IsLocalReturnUrl(url));
    [Fact] public void BackchannelOnlyRewritesExactOrigin()
    {
    ...
    [Fact] public void ConvertsAndRejectsParameters()
    {
    ...
    [Fact] public async Task MapsRowsAndChecksCardinality()
    {
