...
export function createAuthWorker(options: AuthWorkerOptions) {
  return Cloudflare.Worker("Auth", {
    ...
    env: options.email ? { Email: options.email } : {},
    ...
    const auth = yield* BetterAuth({ ...authOptions(publicOrigin, options.client, false, options.features, sendEmail), migrate: false });
    if (!globalThis.__ALCHEMY_RUNTIME__) {
      const { provisionClient } = yield* Effect.promise(() => import("./provision-client.ts"));
      yield* provisionClient(publicOrigin, options.client, options.features);
    }
    return {
      fetch: Effect.gen(function* () {
        ...
        if (path.startsWith("/_flarestack/internal/")) return HttpServerResponse.fromWeb(yield* Effect.promise(() => internalAuth(webRequest, rawAuth, options.features)));
