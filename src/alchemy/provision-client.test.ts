...
test("provisions public PKCE client through provider API", async () => {
 ...
 const options = { ...authOptions("http://localhost:8787", true), database, secret: crypto.randomUUID()+crypto.randomUUID() };
 ...
 try {
 const result = await auth.api.adminCreateOAuthClient({headers, body:{application_type:"native",client_name:"Flarestack Todo",redirect_uris:["http://localhost:8787/signin-oidc"],post_logout_redirect_uris:["http://localhost:8787/signout-callback-oidc"],token_endpoint_auth_method:"none",grant_types:["authorization_code"],response_types:["code"],scope:"openid profile email",skip_consent:true,require_pkce:true,enable_end_session:true}});
 expect(result.client_id).toBe("todo-blazor");
 expect(result.token_endpoint_auth_method).toBe("none");
 await auth.api.adminUpdateOAuthClient({headers,body:{client_id:"todo-blazor",update:{client_name:"Flarestack Todo",skip_consent:true}}});
