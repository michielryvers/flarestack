...
public sealed class AccountClient(HttpClient client, ILogger<AccountClient> logger)
{
    ...
    public async Task<AccountSession?> ValidateAsync(ClaimsPrincipal actor, CancellationToken cancellationToken = default)
    {
        ...
        try {
            ...
            Flarestack.Internal.Protocol.Ensure(response, "Flarestack.Authentication");
            if (!response.IsSuccessStatusCode) return null;
            var session = await response.Content.ReadFromJsonAsync<AccountSession>(cancellationToken);
            return session is not null && session.Id == actor.FindFirstValue("sub") && session.Roles is not null && session.Email is not null && session.Name is not null ? session : null;
        } catch (Flarestack.Internal.ProtocolMismatchException error) { logger.LogError("{ProtocolError}", error.Message); return null; } catch (System.Text.Json.JsonException) { return null; } catch (HttpRequestException) { return null; } catch (TaskCanceledException) when (!cancellationToken.IsCancellationRequested) { return null; }
    }
    internal async Task<UserPage> ListAsync(ClaimsPrincipal actor, string search = "", int offset = 0, CancellationToken cancellationToken = default)
    {
    ...
    internal Task SetRoleAsync(ClaimsPrincipal actor, string userId, string role, CancellationToken cancellationToken = default) => Update(actor,userId,"role",role,cancellationToken);
    internal Task SetDisabledAsync(ClaimsPrincipal actor, string userId, bool disabled, CancellationToken cancellationToken = default) => Update(actor,userId,disabled ? "ban" : "unban",null,cancellationToken);
    internal Task RevokeSessionsAsync(ClaimsPrincipal actor, string userId, CancellationToken cancellationToken = default) => Update(actor,userId,"revoke",null,cancellationToken);
}
